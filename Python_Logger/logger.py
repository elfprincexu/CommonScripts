...
DEFAULT_FMT = '%(asctime)s %(levelname)s [%(filename)s line:%(lineno)d]: %(message)s'
# DEFAULT_DATE_FMT = '%Y-%m-%d %a, %p %H:%M:%S'
DEFAULT_DATE_FMT = '%Y-%m-%d %H:%M:%S'
DEFAULT_LEVEL = 'DEBUG'


def get_logger(loggername='root', cmdlog=True, filelog=True, filename='./log/app.log', filemode='a',
               file_encoding='UTF-8', colorful=True,
...
class Logger():
    ...
    __LOG_ARGS = ['cmdlog', 'cmd_color_dict', 'filelog', 'filename', 'filemode', 'file_encoding', 'colorful',
                  'cmdlevel', 'loggername',
    ...
    def __add_filehandler(self):
        ...
        if self.backup_count == 0:
            self.filehandler = logging.FileHandler(self.filename, self.filemode, self.file_encoding)
        # RotatingFileHandler
        elif not self.when:
            self.filehandler = logging.handlers.RotatingFileHandler(self.filename,
                                                                    self.filemode, self.file_encoding, self.limit,
                                                                    self.backup_count)
        # TimedRotatingFileHandler
        else:
            self.filehandler = logging.handlers.TimedRotatingFileHandler(self.filename, self.file_encoding,
...
if __name__ == '__main__':
    log = get_logger(cmdlog=True, filelog=True, colorful=True, filename=r'./log/test.log')
    log.debug("debug")
    log.info("info")
    log.error("error")
